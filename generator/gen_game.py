...
.teams-preview .tp-team{flex:1; background:var(--bg); border:1px dashed var(--line); border-radius:var(--radius-sm); padding:12px 14px;}
.teams-preview .tp-name{font-family:var(--mono); font-size:11.5px; font-weight:700; text-transform:uppercase; letter-spacing:.03em; color:var(--accent-ink); margin-bottom:6px;}
.teams-preview .tp-players{font-size:14px; color:var(--ink-soft); line-height:1.6;}
.teams-preview .tp-players span{display:block;}
...
(function(){{
  ...
  function splitIntoTeams(raw) {{
    // Snake-alternating assignment (А, Б, А, Б, ...) so both rosters stay
    // as even as possible and it's clear at a glance who plays for whom.
    const rosters = [[], []];
    raw.forEach((name, i) => rosters[i % 2].push(name));
    return rosters;
  }}
  ...
  function buildTeams() {{
    ...
    const rosters = splitIntoTeams(raw);
  ...
  function renderTeamsPreview() {{
    ...
    els.teamsPreview.innerHTML = TEAM_NAMES.map((name, i) => `
      <div class="tp-team">
        ...
        <div class="tp-players">${{(rosters[i].length ? rosters[i] : ['—']).map(n => `<span>${{n}}</span>`).join('')}}</div>
      </div>`).join('');
  }}
  ...
  function renderScoreboard() {{
    ...
    teams.forEach((t, i) => {{
      ...
      const roster = t.players.map((n, pi) => {{
        ...
        return isCurrent ? `<span class="cur">${{n}}</span>` : n;
      }}).join(', ');
      div.innerHTML = `
